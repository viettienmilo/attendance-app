...
import { spss } from '@/assets/spss-program';
  ...
  return (
    <div className='pt-3 md:pt-8 px-8 gap-4 flex flex-col justify-start items-stretch w-full overflow-hidden'>
      <div className='container w-full md:w-8xl justify-center py-8'>
        <div className='text-2xl font-semibold'>{spss.title}</div>
        <div className='w-full flex flex-col-reverse gap-3 lg:grid lg:grid-cols-[1fr_50px_1.5fr] mt-8 items-center'>
          <blockquote className='border-l-4 border-teal-500 pl-4 italic text-muted-foreground my-4'>
            {spss.brief}
          </blockquote>
          <div></div>
          <AspectRatio
        ...
        <ul className='space-y-3 ml-6 my-3'>
          {[spss.intro1, spss.intro2, spss.intro3, spss.intro4].map(
            (text, index) => (
              <li
                key={index}
        ...
        <ul className='space-y-3 ml-6 my-3'>
          {[spss.hardskill, spss.softskill].map((text, index) => (
            <li
              key={index}
              className='flex items-start gap-2.5 text-sm'
            >
        ...
        <ul className='space-y-3 ml-6 my-3'>
          {spss.lesson_contents.map((chapter, index) => {
        ...
        <div className='text-xl mt-8'>THỰC HÀNH</div>
        <ul className='space-y-3 ml-6 my-3'>
          {[spss.test_intro1].map((text, index) => (
            <li
              key={index}
              className='flex items-start gap-2.5 text-sm'
            >
